pipeline {
    ...
    stages {
        stage('Build') {
            ...
            parallel {
                stage('Build app1') {
                    steps {
                        echo "Building app1"
                        sh "docker-compose -p ${env.BUILD_ID} up --build --no-start app1"
                ...
                stage('Build app2') {
                    steps {
                        echo "Building app2"
                        sh "docker-compose -p ${env.BUILD_ID} up --build --no-start app2"
                    }
                }
            }
        }
        ...
        stage('Push') {
            steps {
                // todo - in parallel
                sh "docker-compose -p ${env.BUILD_ID} push"
            }
        }

        stage('Deploy') {
            steps {
                sh "docker-compose -p ${env.BUILD_ID} up -d"
            }
        }
